...
{
    [TestFixture]
    public class UInt32PointerTest : AssertionHelper
    {
        private Random random = new Random();

        public uint GenerateRandomNumber()
        {
            return (uint)random.Next(0, Int32.MaxValue);
        }
        ...
        public unsafe void StackallocTest1()
        {
            ...
            uint* sample = stackalloc uint[bufferSize];
            UInt32Pointer pointer = new UInt32Pointer(sample);
            uint[] results = new uint[bufferSize];
        ...
        public unsafe void StackallocTest6()
        {
            ...
            // Pointer conversion test
            for (int i = 0; i < bufferSize; i++)
                results[i] = *(uint*)(pointer + i) = GenerateRandomNumber();
        ...
        {
            uint* sample = stackalloc uint[4];

            int totalSize = 0;

            int ptrSize1 = Marshal.SizeOf(new UInt32Pointer(sample));
            Console.WriteLine("Marshal.SizeOf(new UInt32Pointer(...)): {0}", ptrSize1);
            totalSize += ptrSize1;

            int ptrSize2 = Marshal.SizeOf(typeof(UInt32Pointer));
            Console.WriteLine("Marshal.SizeOf(typeof(UInt32Pointer)): {0}", ptrSize2);
            totalSize += ptrSize2;
            ...
            int ptrSize5 = Marshal.SizeOf(typeof(uint*));
            Console.WriteLine("Marshal.SizeOf(typeof(uint*)): {0}", ptrSize5);
            totalSize += ptrSize5;

            Assert.AreEqual(totalSize, UInt32Pointer.Size * 5);
        }
        ...
        public unsafe void SizeOfTest2()
        {
            ...
            int ptrSize1 = sizeof(UInt32Pointer);
            Console.WriteLine("sizeof(UInt32Pointer): {0}", ptrSize1);
            totalSize += ptrSize1;
            ...
            int ptrSize3 = sizeof(uint*);
            Console.WriteLine("sizeof(uint*): {0}", ptrSize3);
            totalSize += ptrSize3;

            Assert.AreEqual(totalSize, UInt32Pointer.Size * 3);
        }
        ...
        public unsafe void EqualityTest1()
        {
            ...
            UInt32Pointer address3 = new UInt32Pointer(address2);
            Console.WriteLine("UInt32Pointer Address (from IntPtr): {0:X}", address3.ToInt32());
            checksum += address3.ToInt32();

            UInt32Pointer address4 = new UInt32Pointer(address1);
            Console.WriteLine("UInt32Pointer Address (from Int32): {0:X}", address4.ToInt32());
