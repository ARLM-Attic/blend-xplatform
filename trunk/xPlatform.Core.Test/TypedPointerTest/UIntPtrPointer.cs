...
{
    [TestFixture]
    public class UIntPtrPointerTest : AssertionHelper
    {
        private Random random = new Random();

        public UIntPtr GenerateRandomNumber()
        {
            return new UIntPtr((uint)random.Next(0, Int32.MaxValue));
        }
        ...
        public unsafe void StackallocTest1()
        {
            ...
            UIntPtr* sample = stackalloc UIntPtr[bufferSize];
            UIntPtrPointer pointer = new UIntPtrPointer(sample);
            UIntPtr[] results = new UIntPtr[bufferSize];
        ...
        public unsafe void StackallocTest6()
        {
            ...
            // Pointer conversion test
            for (int i = 0; i < bufferSize; i++)
                results[i] = *(UIntPtr*)(pointer + i) = GenerateRandomNumber();
        ...
        public unsafe void SizeOfTest2()
        {
            ...
            int ptrSize1 = sizeof(UIntPtrPointer);
            Console.WriteLine("sizeof(UIntPtrPointer): {0}", ptrSize1);
            totalSize += ptrSize1;
            ...
            int ptrSize3 = sizeof(UIntPtr*);
            Console.WriteLine("sizeof(UIntPtr*): {0}", ptrSize3);
            totalSize += ptrSize3;

            Assert.AreEqual(totalSize, UIntPtrPointer.Size * 3);
        }
        ...
        {
            UIntPtr* sample = stackalloc UIntPtr[4];
            UIntPtrPointer a = new UIntPtrPointer(sample);
            UIntPtrPointer b = (a + 1);
            Console.WriteLine("Address offset: {0}", b.ToInt32() - a.ToInt32());

            Assert.AreEqual(sizeof(UIntPtr), b.ToInt32() - a.ToInt32());
            Assert.False(Object.ReferenceEquals(a, b));

            // xPlatform's typed pointers are value type.
            UIntPtrPointer c = new UIntPtrPointer(sample + 1);
            UIntPtrPointer d = (++c);
