...
{
    [TestFixture]
    public class TimeSpanPointerTest : AssertionHelper
    {
        private MersenneTwister64 random = new MersenneTwister64();

        public TimeSpan GenerateRandomNumber()
        {
            return new TimeSpan(random.NextInt64());
        }
        ...
        public unsafe void StackallocTest6()
        {
            ...
            // Pointer conversion test
            for (int i = 0; i < bufferSize; i++)
                results[i] = *(TimeSpan*)(pointer + i) = GenerateRandomNumber();
